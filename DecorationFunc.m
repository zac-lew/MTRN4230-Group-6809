function Detection
    ...
    if (~useRobotCellCamera)
        disp('---USING SAVED IMAGE---');      
        customerImage = imread('PnpTest1.jpg'); 
        %customerImage = imread('PnpTest2.jpg'); 
    else
        %for robot cell (GUI). Returns raw image from robot cell camera
        % needs Image Image Acquisition Toolbox
...
% Open a TCP connection to the robot.
    socket_1 = tcpip(robot_IP_address, robot_port);
    set(socket_1, 'ReadAsyncMode', 'continuous');
    fopen(socket_1);
